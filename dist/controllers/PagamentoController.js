...
async function transacaoPaga(idTransacao, descricao, idUsuario, ignorarValidacaoValor = true) {
    ...
    try {
        ...
        const transacao = await Transacao_1.Transacao.findOne({ where: { id: idTransacao }, transaction });
        if (!transacao) {
...
module.exports = {
    async pagamento(req, res, next) {
        ...
        try {
            ...
            if (evento?.idProdutor === 1) {
                const applicationFee = parseFloat(String(transacao?.taxaServico ?? 0));
                if (applicationFee > 0) {
    ...
    async webHookPagamentoPos(req, res) {
        const { data } = req.body;
    ...
    async pagamentoPOSStone(req, res) {
        try {
     ...
     */
    async ajustarQuantidadePdv(req, res, next) {
        const transaction = await database_1.default.transaction();
        try {
            const { idTransacao, idUsuarioPDV, itens } = req.body;
            if (!idTransacao || !idUsuarioPDV) {
                throw new customError_1.CustomError('idTransacao e idUsuarioPDV são obrigatórios.', 400, '');
            }
            if (!Array.isArray(itens) || itens.length === 0) {
                throw new customError_1.CustomError('Informe os itens a ajustar.', 400, '');
            }
            ...
            if (transacao.status === 'Pago' || transacao.status === 'Cancelado') {
                throw new customError_1.CustomError('Não é possível ajustar quantidade de uma transação finalizada.', 400, '');
            }
            const idsRemovidos = [];
            ...
            for (const item of itens) {
                ...
                if (ids.length === 0) {
                    throw new customError_1.CustomError('Cada item deve informar idsIngressoTransacao.', 400, '');
                }
                if (!Number.isFinite(quantidade) || quantidade < 1) {
                    throw new customError_1.CustomError('A quantidade mínima por tipo de ingresso é 1.', 400, '');
                }
                if (quantidade > ids.length) {
                    throw new customError_1.CustomError('Não é permitido aumentar a quantidade no pagamento PDV.', 400, '');
                }
                if (quantidade === ids.length) {
                    continue;
                }
                const registrosGrupo = await Transacao_1.IngressoTransacao.findAll({
                    where: {
                        idTransacao,
                        id: ids,
                    },
                ...
                if (registrosGrupo.length !== ids.length) {
                    throw new customError_1.CustomError('Itens da transação inválidos ou não pertencem a esta transação.', 400, '');
                }
                const idsParaRemover = ids.slice(quantidade);
                const removidos = registrosGrupo.filter((reg) => idsParaRemover.includes(reg.id));
                for (const reg of removidos) {
                    await Ingresso_1.HistoricoIngresso.create({
                        idIngresso: reg.idIngresso,
                        data: new Date(),
                        descricao: 'Cancelado - quantidade reduzida no Pagamento PDV',
                        idUsuario: idUsuarioPDV,
                    }, { transaction });
                    await Ingresso_1.Ingresso.update({ status: 'Cancelado' }, { where: { id: reg.idIngresso }, transaction });
                    await reg.destroy({ transaction });
                    idsRemovidos.push(reg.id);
                    totalRemovidos += 1;
                }
            }
            const ingressosRestantes = await Transacao_1.IngressoTransacao.findAll({
                where: { idTransacao },
                transaction,
            });
            if (ingressosRestantes.length === 0) {
                throw new customError_1.CustomError('A transação deve manter ao menos 1 ingresso.', 400, '');
            }
            const preco = ingressosRestantes.reduce((acc, ingresso) => acc + Number(ingresso.preco || 0), 0);
            const taxaServico = ingressosRestantes.reduce((acc, ingresso) => acc + Number(ingresso.taxaServico || 0), 0);
            const taxaServicoDesconto = ingressosRestantes.reduce((acc, ingresso) => acc + Number(ingresso.taxaServicoDesconto || 0), 0);
            const valorTotal = ingressosRestantes.reduce((acc, ingresso) => acc + Number(ingresso.valorTotal || 0), 0);
            await Transacao_1.Transacao.update({
                preco,
                taxaServico,
                taxaServicoDesconto,
                valorTotal,
            ...
            await transaction.commit();
            let transacaoAtualizada = await Transacao_1.Transacao.findOne({
                where: { id: idTransacao },
            });
            ...
            if (transacaoAtualizada &&
                ...
                await transacaoPaga(idTransacao, 'Venda quitada no Pagamento PDV após redução de quantidade', idUsuarioPDV, false);
                transacaoAtualizada = await Transacao_1.Transacao.findOne({
                    where: { id: idTransacao },
                });
