...
module.exports = {
    ...
    async enviarIngressoWhatsApp(req: any, res: any, next: any) {
        try {
            ...
            const registro = await Ingresso.findByPk(idIngresso);
            if (!registro) {
                throw new CustomError('Ingresso não encontrado.', 404, '');
            }
            const evento = await Evento.findByPk(registro.idEvento);

            let user;
            try {
                user = await Usuario.findByPk(registro.idUsuario);
                const url = `https://jangoingressos.com.br/ingresso?qrcode=${registro.qrcode}`;
                const options = {
                    method: "POST",
                    headers: {
